...
module.exports = {
  ...
  async execute(message, args, client, config) {
    ...
    try {
      const reply = await message.reply('🔄 Attempting to update profile picture... This may take a moment.');
      ...
      if (result.success) {
        ...
        logger.warn({ userId: message.author.id, error: result.error, ratelimited: result.ratelimited }, 'PFP update failed for owner.');
      }
    } catch (error) {
      logger.error({ error, command: 'pfp', userId: message.author.id }, 'Error executing pfp command');
      await message.reply('❌ An unexpected error occurred while trying to update the profile picture.');
    }
  },
};
