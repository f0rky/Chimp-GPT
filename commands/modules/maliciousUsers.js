  ...
  try {
    ...
    if (stats.recentDeletions.length > 0) {
      const recent = stats.recentDeletions
        .map(d => `${new Date(d.timestamp).toLocaleString()}: "${d.content.substring(0, 50)}${d.content.length > 50 ? '...' : ''}"`)
        .join('\n');
      embed.fields.push({ name: 'Recent Deletions', value: recent.substring(0, 1024), inline: false });
    }
...
async function getDetectionConfig(message, args) {
  try {
    ...
    const embed = {
      ...
      fields: [
        { name: 'Max Deletions/Hour', value: config.MAX_DELETIONS_PER_HOUR.toString(), inline: true },
        { name: 'Max Deletions/Day', value: config.MAX_DELETIONS_PER_DAY.toString(), inline: true },
        { name: 'Rapid Delete Threshold', value: `${config.RAPID_DELETE_THRESHOLD_MS / 1000}s`, inline: true },
