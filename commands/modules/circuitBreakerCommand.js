...
const circuitBreakerCommand = {
    ...
    .addSubcommand(subcommand =>
      subcommand
        ...
        .addStringOption(option =>
          option.setName('id')
            .setDescription('The approval ID to approve')
            .setRequired(true)))
    .addSubcommand(subcommand =>
      subcommand
        .setName('deny')
        ...
        .addStringOption(option =>
          option.setName('id')
            .setDescription('The approval ID to deny')
            .setRequired(true)))
    .addSubcommand(subcommand =>
      subcommand
        .setName('list')
        .setDescription('List all pending approval requests'))
    .addSubcommand(subcommand =>
      subcommand
        .setName('status')
        .setDescription('Check the circuit breaker status'))
    .addSubcommand(subcommand =>
      subcommand
        .setName('version')
  ...
  async execute(interaction, config, args = []) {
    ...
    if (isSlashCommand) {
      ...
      // Handle text command
      if (!args.length) {
        await interaction.reply('Please specify a subcommand: approve, deny, list, status, version');
        return;
      }
      ...
      switch (subcommand) {
        ...
        case 'version':
          ...
          break;
        default:
          await interaction.reply('Unknown subcommand. Please use one of: approve, deny, list, status, version');
      }
    }
  }
};
