...
function generateStartupReport() {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: '💾 Conversation Storage', value: 
 ...
 */
async function sendOwnerStartupReport(client) {
  try {
    // Generate the startup report
    ...
    logger.info('Startup report sent to owner');
    return true;
  } catch (error) {
    logger.error({ error }, 'Failed to send startup report to owner');
    return false;
  }
}
