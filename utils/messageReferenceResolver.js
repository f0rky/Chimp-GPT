...
async function resolveReference(message) {
  ...
  try {
    logger.debug({ referenceId, channel: message.channel?.id }, 'Fetching referenced message from Discord API');
    ...
    logger.error({ error, messageId: message.reference.messageId }, 'Failed to resolve message reference');
    return null;
  }
}
