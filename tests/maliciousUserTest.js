...
async function testUserBlockingAndUnblocking() {
  ...
  assert(
    maliciousUserManager.isUserBlocked(userId) === false,
  ...
  assert(
    maliciousUserManager.isUserBlocked(userId) === false,
    'User should not be blocked after unblocking'
  );
...
async function testPersistence() {
  ...
  assert(
    statsAfter.totalDeletions === statsBefore.totalDeletions,
    'Total deletions should be persisted'
  );
  assert(
    statsAfter.rapidDeletions === statsBefore.rapidDeletions,
    'Rapid deletions should be persisted'
  );
  assert(statsAfter.isBlocked === statsBefore.isBlocked, 'Blocked status should be persisted');
  assert(
    maliciousUserManager.isUserBlocked(userId) === true,
    'User should still be blocked after restart'
  );
...
async function testDetectionConfiguration() {
  ...
  assert(
    typeof config.RAPID_DELETE_THRESHOLD_MS === 'number',
    'Should have RAPID_DELETE_THRESHOLD_MS'
  );
  ...
  assert(
    config.MAX_DELETIONS_PER_DAY > config.MAX_DELETIONS_PER_HOUR,
    'Daily limit should be higher than hourly'
  );
  assert(
    config.RAPID_DELETE_THRESHOLD_MS < 60000,
    'Rapid delete threshold should be under 1 minute'
  );
