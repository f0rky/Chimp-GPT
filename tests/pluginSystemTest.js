...
async function testPluginSystem() {
  ...
  try {
    ...
    try {
      ...
      fs.writeFileSync(tempPluginFile, `
      ...
      if (!test2Result.success) {
        ...
        logger.info({ errorMessage: validationError.message }, 'Plugin validation working correctly');
      }
    } catch (error) {
    ...
    try {
      ...
      if (!test3Result.success) {
        ...
        logger.info({ commandCount: Object.keys(commands).length }, 'Commands registered successfully');
      }
    } catch (error) {
    ...
    try {
      ...
      if (!test4Result.success) {
        ...
        logger.info({ functionCount: Object.keys(functions).length }, 'Functions registered successfully');
      }
    } catch (error) {
