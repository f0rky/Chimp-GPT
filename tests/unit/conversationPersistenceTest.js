...
async function testConversationPersistence() {
  ...
  try {
    ...
    try {
      ...
      const conversation1Loaded =
        loadedConversationsMap.has(testUserId1) &&
        loadedConversationsMap
          .get(testUserId1)
          .some(msg => msg.role === 'user' && msg.content === 'Hello, bot!') &&
        loadedConversationsMap
          .get(testUserId1)
          .some(msg => msg.role === 'assistant' && msg.content === 'Hello, human!');

      const conversation2Loaded =
        loadedConversationsMap.has(testUserId2) &&
        loadedConversationsMap
          .get(testUserId2)
          .some(msg => msg.role === 'user' && msg.content === 'How are you?') &&
        loadedConversationsMap
          .get(testUserId2)
          .some(msg => msg.role === 'assistant' && msg.content === 'I am fine, thank you!');
